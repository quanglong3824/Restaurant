...
if ($hasItems) {
    foreach ($orderItems as $oi) {
        if ($oi['status'] !== 'cancelled')
            $orderTotal += $oi['item_price'] * $oi['quantity'];
    }
}
?>
...
<style>
    ...
    .bill-item-status.confirmed {
        color: #10b981;
    }

    .bill-item-status.pending {
        color: #f59e0b;
    }

    .bill-item-status.draft {
        color: #94a3b8;
    }
    ...
    @keyframes pulseGold {
        0% {
            box-shadow: 0 0 0 0 rgba(197, 160, 89, .7);
        }
...
<div class="customer-menu-wrapper" id="menuWrapper" style="display:none;">
    ...
    <header class="menu-header">
        ...
        <div class="ctx-banner"
            style="color:<?= $contextColor ?>;background:<?= $contextColor ?>15;border-color:<?= $contextColor ?>44;">
    ...
    <?php if (count($presentTypes) > 1): ?>
        <div class="type-tab-bar" id="typeTabBar">
            ...
            <?php foreach ($presentTypes as $tp):
                if (!isset($typeLabels[$tp]))
                    continue; ?>
    ...
    <div class="menu-search-container">
        <div class="menu-search-bar">
            ...
            <button id="btnClearSearch"
                style="display:none;background:none;border:none;color:#94a3b8;cursor:pointer;padding:0 4px;"
    ...
    <main class="menu-sections" id="menuSections">
            ...
            <section class="menu-section" id="cat-<?= $cat['id'] ?>" data-type="<?= $cat['menu_type'] ?>">
                ...
                <div class="menu-list">
                    <?php foreach ($grouped[$cat['id']] as $item):
                        ...
                        ?>
                        <div class="menu-item-card<?= $isUnavailable ? ' item-unavailable' : '' ?>" data-id="<?= $item['id'] ?>"
                            data-name="<?= strtolower(e($item['name'])) ?>"
                            data-name-en="<?= strtolower(e($item['name_en'] ?? '')) ?>" data-price="<?= $item['price'] ?>"
                            data-type="<?= $cat['menu_type'] ?>" data-options="<?= e($item['note_options'] ?? '') ?>"
                                ...
                                <?php if (!empty($tags)): ?>
                                    <div class="item-tags">
                                        <?php foreach ($tags as $tag):
                                            if (!in_array($tag, ['bestseller', 'new', 'spicy', 'vegetarian', 'recommended']))
                                                continue; ?>
        ...
        <div id="searchNoResult" class="menu-empty-state" style="display:none;">
            ...
            <button onclick="clearMenuSearch()"
                style="background:none;border:1.5px solid #e2e8f0;border-radius:20px;padding:8px 20px;cursor:pointer;font-weight:600;color:#64748b;margin-top:8px;">
                Xoá tìm kiếm
            </button>
        </div>
    </main>
    ...
    <div class="modal modal-bottom">
        ...
        <div class="modal-body">
            ...
            <div class="order-notes-box mt-3" style="margin-top:1rem;">
                <label
                    style="font-size:.72rem;font-weight:800;color:#94a3b8;text-transform:uppercase;letter-spacing:1px;">GHI
                    CHÚ ĐƠN HÀNG</label>
...
<div id="itemDetailModal" class="modal-backdrop hidden">
    <div class="modal modal-bottom modal-premium">
        ...
        <div class="item-detail-img" id="detailImg"
            style="width:100%;height:240px;background-size:cover;background-position:center;position:relative;"></div>
        <div class="modal-body">
            <div style="margin-bottom:1rem;">
                ...
                <div id="detailPrice" class="item-price"
                    style="font-size:1.2rem;font-weight:800;color:var(--gold-dark);"></div>
                <p id="detailDesc" class="item-desc"
                    style="margin-top:8px;font-size:.875rem;color:#64748b;line-height:1.5;"></p>
            </div>
            <div id="detailOptsWrap" style="display:none;margin-bottom:1.25rem;">
                <label
                    style="font-size:.72rem;font-weight:800;color:#94a3b8;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;display:block;">
